...
export default function Vacancy_create () {
    ...
    useEffect(()=>{
        ...
        data =>{
            ...
            setChecked(data.vacancydata.isActive)
    ...
    const axiosPostData = async() => {
        ...
        await axios.post('http://localhost:5000/company/vacancy/edit/'+params.vid, postData)
    ...
    const handleSubmit = (e) => {
        ...
        if (!title || !salary || !description || !contacts || !competencies) {
            setError(<p>Name is empty. Please type a Name.</p>)
        } else {
            setError('')
            axiosPostData()
    ...
    return (
        ...
        {error}
        <input className="btn vacancy-create-btn" type="submit" value="Оновити дані" onClick={handleSubmit} />
     </form>
